...
$modelo_aluguer_query = "SELECT modelo FROM `vehiculo_venda` WHERE modelo='$modelo'";
$resultado = mysqli_query($db, $modelo_aluguer_query);
  //Gárdase o resultado da query en $resultado e compróbase
  //o número de filas. Se este é menor que 1 salta erro (o modelo non existe na táboa vehiculo_venda)
...
//a mostralos todos por pantalla
  foreach ($erros as $valor){
    ...
    mysqli_close($db);
  ?>
  <p><a href="modificar_venda.php">Volver ao formulario de modificación dun vehículo en venda</a></p>
  ...
  if($cantidade != ""){
    $query = "UPDATE vehiculo_venda SET cantidade='$cantidade' WHERE `modelo`='$modelo'";
    mysqli_query($db, $query);
  }

  if($descricion != ""){
    $query = "UPDATE vehiculo_venda SET descricion='$descricion' WHERE `modelo`='$modelo'";
    mysqli_query($db, $query);
  }

  if($marca != ""){
    $query = "UPDATE vehiculo_venda SET marca='$marca' WHERE `modelo`='$modelo'";
    mysqli_query($db, $query);
  }

  if($prezo != ""){
    $query = "UPDATE vehiculo_venda SET prezo='$prezo' WHERE `modelo`='$modelo'";
    mysqli_query($db, $query);
  }

  if($foto != ""){
    $query = "UPDATE vehiculo_venda SET foto='$foto_SQL' WHERE `modelo`='$modelo'";
    mysqli_query($db, $query);
  }
